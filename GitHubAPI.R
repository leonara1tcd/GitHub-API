...
myData = GET("https://api.github.com/users/mbostock/followers?per_page=100;",
             gtoken)
...
for (i in 1:length(user_ids))
{
  ...
  for (j in 1:length(followingLogin))
  {
    if (is.element(followingLogin[j], users) == FALSE)
    {
      ...
      yearCreated = substr(followingDF2$created_at,
                           start = 1,
                           stop = 4)
      
      #Add users data to a new row in dataframe
      usersDB[nrow(usersDB) + 1,] = c(followingLogin[j],
                                      followingNumber,
                                      followersNumber,
                                      reposNumber,
                                      yearCreated)
...
plot2 = plot_ly(
  ...
  x = ~ following,
  y = ~ followers,
  text = ~ paste("Followers: ", followers, "<br>Following: ", following),
  color = ~ dateCreated
)
plot2
...
#Plot 3 shows the 10 most popular languages used by mbostock's followers
...
for (i in 1:length(users))
{
  ...
  {
    RepositoriesUrl2 = paste("https://api.github.com/repos/",
                             users[i],
                             "/",
                             RepositoriesNames[j],
                             sep = "")
...
plot3 = plot_ly(
  data = languageDF,
  x = languageDF$languages,
  y = languageDF$Freq,
  type = "bar"
)
plot3

#Sends graph to plotly - located at https://plot.ly/~leonara1/7/#/
api_create(plot3, filename = "Ten Most Popular Languages")
